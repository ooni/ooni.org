...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (dateStr) {
      // Firefox cannot parse freeform strings like "August 2025" via the
      // Date constructor. Extract month name and year explicitly instead.
      ...
      if (m) {
        ...
        if (monthIdx !== undefined && !Number.isNaN(yr)) {
...
function focusTag(focus) {
  if (focus.includes('report'))
    return '<span class="tag-focus report">OONI Report</span>';
  if (focus.includes('Raw'))
    return '<span class="tag-focus data">Raw Data</span>';
  if (focus.includes('Promoting'))
    return '<span class="tag-focus tools">Promoting Tools</span>';
...
function linkCell(link, archiveLink) {
  ...
  if (archiveLink) {
    html +=
      ' <a href="' +
      archiveLink +
      '" target="_blank" rel="noopener" style="font-size:0.7rem;color:#adb5bd;">[archive' +
      EXT_ICON +
      ']</a>';
  } else {
    html +=
...
function createOoniTable(config) {
  ...
  function render() {
    ...
    document.getElementById(ns + '-count').textContent = config.countLabel(
      filtered.length,
    );
    const start = state.page * state.perPage;
    document.getElementById(ns + '-tbody').innerHTML = sorted
      .slice(start, start + state.perPage)
      .map(config.rowHtml)
      .join('');
    renderPagination(filtered.length);
    document
      .querySelectorAll('#' + ns + '-table th[data-sort]')
      .forEach((th) => {
  ...
  function renderPagination(total) {
    ...
    html +=
      ...
      '.next()">Next</button>';
    html +=
      '<span class="page-info">' + (state.page + 1) + ' / ' + pages + '</span>';
    el.innerHTML = html;
  }
  ...
  fetch(CSV_URL)
    ...
    .then((text) => {
      ...
      document.querySelectorAll('#' + ns + '-controls select').forEach((s) =>

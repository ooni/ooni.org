...
* [Блокировка HTTP](#блокировка-http)
...
* [Блокировка TCP/IP](#блокировка-tcpip)
...
Данные BGP агрегированны и опубликованны такими организациями, как [RIPE](https://www.ripe.net/) и [CAIDA](https://www.caida.org/), и могут быть полезны при обнаружении перебоев в работе интернета.
...
Некоторые правительства время от времени публикуют официальные блок-листы (или они попадают в утечки данных), содержащие списки сайтов, которые законодательно запрещены в стране.

Интернет-провайдеры получают распоряжение блокировать доступ ко всем сайтам, включенным в такие блокирующие списки, которые обычно включают сотни (или тысячи) URL-адресов, содержащих контент, запрещенный в данной
...
Домен -- это имя, которое обычно присваивается сайтам (когда они создаются), чтобы их было легче найти и запомнить. Например, twitter.com -- это домен сайта Twitter.
...
В результате вы либо получаете IP-адрес заблокированной страницы, либо получаете ответ, утверждающий, что доменного имени не существует.
...
DNS-регистратор управляет DNS-запросами (для всех клиентов в сети) и отвечает за преобразование имен хостов (/доменов) (например, twitter.com) в IP-адреса (например, 38.229.72.16).
...
Это *не включает* в себя [неправильную конфигурацию DNS](https://ooni.org/post/not-quite-network-censorship/) и не похоже на перехват DNS, когда DNS-регистратор отвечает поддельным ответом.
...
Например, **twitter.com** -- это домен сайта Twitter.
...
* **Отображение блокирующей страницы**: в этом случае провайдер перехватывает запрос пользователя на доступ к определенному сайту (например, `facebook.com`) и вместо него выдает пользователю страницу блокировки, которая информирует пользователя о том, что ему запрещен доступ к запрашиваемому сайту.
...
Однако в некоторых случаях интернет-провайдеры не позволяют пользователям получить доступ к определенным сайтам, блокируя или мешая соединению между ними и сервером. Это одна из форм блокировки HTTP, [измеряемая](https://ooni.org/nettest/web-connectivity/) приложением [OONI Probe](https://ooni.org/install/).
...
Когда пользователь делает запрос к серверу (например, для доступа к facebook.com), прозрачный прокси перехватывает запрос для выполнения различных действий (таких как кэширование, перенаправление и
аутентификация).
...
Для мониторинга отключений интернета существует [несколько открытых источников данных](https://ooni.org/post/examining-internet-blackouts/), например, [IODA](https://ioda.caida.org/).
...
[Onion-сервисы](https://community.torproject.org/onion-services/) (ранее известные как «скрытые сервисы Tor») -- это интернет-сервисы, доступ к которым возможен только [через сеть Tor](https://www.torproject.org/), что обеспечивает их пользователям повышенную конфиденциальность и безопасность.

Например, сайты могут быть помещены в анонимное сетевое местоположение с помощью onion-сервисов, что позволяет им скрыть свой IP-адрес и обеспечить большую конфиденциальность для своих посетителей. Такие сайты можно отличить по их .onion адресам, например, `https://facebookcorewwwi.onion/` или `http://www.nytimes3xbfgragh.onion/`.

В течение последних лет [версия OONI Probe для Linux и macOS](https://ooni.org/install/ooniprobe/) использовала
onion-сервисы для загрузки измерений на серверы OONI.
...
OONI Probe разрабатывается [Открытой обсерваторией сетевого вмешательства](https://ooni.org/) с 2012 года и доступно на [Android](https://play.google.com/store/apps/details?id=org.openobservatory.ooniprobe), [iOS](https://itunes.apple.com/us/app/id1199566366), [Linux](https://github.com/ooni/probe-cli/releases), [Windows](https://github.com/ooni/probe-desktop/releases) и [macOS](https://github.com/ooni/probe-desktop/releases).

### OONI Run

[OONI Run](https://run.ooni.io/) -- это веб-интерфейс, который позволяет пользователям мобильного приложения OONI Probe генерировать ссылки (пользовательская схема URI) для **кастомизированного тестирования OONI Probe**.
...
При общении с машиной по протоколу TCP вы используете пару IP и порт, которая выглядит примерно следующим образом: 10.20.1.1:8080.
...
* **Глобальный список для тестирования**: включает широкий спектр сайтов, имеющих международное значение (например, facebook.com), большинство из которых на английском языке.
...
Когда вы подключаетесь к сайту с помощью TLS, адрес сайта начинается с HTTPS (например, `https://www.facebook.com/`), а не с HTTP.
...
Например, `https://twitter.com/` -- это URL, а twitter.com -- домен.
